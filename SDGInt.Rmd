...
##FIGURE 7: Proportion of synergy, tradeoff and unspecified interactions disaggregated into individual SDG 15 indicator
```{r}

alldat = read.csv('Figure7.csv', header = TRUE, sep = ",")

alldat$indicator = factor(alldat$indicator, levels = c("primary_forest", "kba_freshwater", "kba_mountain", "redlist", "biodiversityaid"))
...
panel_fig7 = ggplot(alldat, aes(fill = interaction,y = count, x = indicator, label = scales::percent(count))) + geom_bar(position = "stack", stat = "identity") + 
  scale_fill_manual(values = c("#ffff99" , "#fdc086", "#7fc97f")) +
  theme(axis.text.x = element_text(size = 8)) +
  labs(y = "", x = "", title = "(c) Proportion of interactions between SDG 15 indicators and other\nSDGs") + 
  scale_y_continuous(labels = function(x) paste0(x*100, "%")) + 
  geom_text(size = 3, position = position_stack(vjust = 0.5)) + 
...
##Combining figures 4, 5 and 7 together
```{r}
combined = ggarrange(panel_fig5, ggarrange(panel_fig4, panel_fig7, nrow = 1, ncol = 2, common.legend = TRUE, legend  = "bottom"), nrow = 2, ncol = 1)
combined
```



##FIGURE 8: An indicator level network diagram of SDGs
```{r}
...
E(network)$width = abs(E(network)$interaction)*6
...
#renaming vertices
vname = read.csv('D:/2. GMBA/3. Exploratory Data Analysis/3. New figures 23 Sept/Network/indicator_sdg.csv', header = TRUE, sep = ",")
...
plot(network,
     ...
     vertex.label.cex = 2.5, #size of the labels
...
##FIGURE 9: Network diagram for interactions between SDG 15 indicators and the other SDGs
```{r}
...
#giving width to edges according to their magnitudes
E(network2)$width = abs(E(network2)$interaction)*20
...
x = c(2,3,2.7,2,1,1.2,2,1,1.4,2.2,2.1,1.4,1,1.3,1.9,2.4,2.2,2.7,2.5,3) #to adjust size of labels according to vertex size
...
plot(network2,
     ...
     vertex.label.cex = 1.6, #size of the labels
     vertex.label.color = 'black',
     vertex.label.dist = x,
     vertex.label.font = 2, #making the label bold
     edge.curved = 0.1,
